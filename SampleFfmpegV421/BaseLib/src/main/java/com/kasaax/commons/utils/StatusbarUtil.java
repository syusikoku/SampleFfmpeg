...
public class StatusbarUtil {
     ...
     * 修改状态栏颜色，支持4.4以上版本
     */
    public static void setStatusBarColor(Activity _activity, int _colorId) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            Window window = _activity.getWindow();
            window.setStatusBarColor(_activity.getResources().getColor(_colorId));
        }
    }
...
//        }
    ...
    public static boolean MIUISetStatusBarLightMode2(Window window, boolean light) {
        if (window == null) {
            return false;
        }
        boolean result = false;
...
//            window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
     ...
     * @return boolean 成功执行返回true
     */
    public static boolean FlymeSetStatusBarLightMode2(Window window, boolean light) {
        boolean result = false;
        if (window != null) {
    ...
    private static void initStatusBarHeight(Context context) {
        ...
        if (DeviceUtils.isTablet(context)
                && sStatusbarHeight > Utils.dp2px(context, STATUS_BAR_DEFAULT_HEIGHT_DP)) {
